-- // VoidHub | Version : 0.3.0 | Added: Separator, SubPage, MultiColumn, PageSearch
-- // VoidHub UI Library by vonplayz_real
...
local BASE = "https://raw.githubusercontent.com/VoidDeveloper67/VoidHub-Library/refs/heads/main/"
...
local function _MakeColorPicker(Config, SectionAdd, CountItem, AccentColor, MainDropShadow)
    ...
    if not _ColorpickerModule then
        warn("[VoidHub] Colorpicker.lua gagal dimuat")
        return {}
    end
...
function Chloex:Window(GuiConfig)
    ...
    GuiConfig.Config.ConfigFolder = GuiConfig.Config.ConfigFolder or "VoidHub/Config/"
    ...
    if ks then
        ...
        if not ok or type(result) ~= "function" then
            warn("[VoidHub] KeySystem gagal dimuat")
        else
            local ok2, resolved = pcall(result, ks, GuiConfig, CoreGui, TweenService, getIconId)
            if not ok2 then
                warn("[VoidHub] KeySystem error: " .. tostring(resolved))
    ...
    function GuiFunc:SaveConfig(name)
        if not writefile then warn("[VoidHub] writefile tidak tersedia") return false end
        _checkFolders()
        _ConfigData._version = GuiConfig.Version
        local ok, err = pcall(function()
            writefile(_cfgFile(name), HttpService:JSONEncode(_ConfigData))
        end)
        if not ok then warn("[VoidHub] SaveConfig error:", err) end
        return ok
    end

    function GuiFunc:LoadConfig(name)
        if not isfile or not readfile then warn("[VoidHub] isfile/readfile tidak tersedia") return false end
        _checkFolders()
    ...
    if GuiConfig.BackgroundVideo and GuiConfig.BackgroundVideo ~= "" then
        task.spawn(function()
            ...
            if isAssetId then
                ...
                local fileName = "VoidHub_bgvideo.webm"
                if writefile and getcustomasset and isfile then
                    ...
                    warn("[VoidHub] BackgroundVideo: writefile/getcustomasset tidak tersedia")
                    return
                end
            end
    ...
    function GuiFunc:DestroyGui()
        if CoreGui:FindFirstChild("VoidHub") then Chloeex:Destroy() end
    end
    ...
    function GuiFunc:Notify(Config)
        ...
        local NotifGui = CoreGui:FindFirstChild("VoidHub_Notifs")
        if not NotifGui then
            NotifGui = Instance.new("ScreenGui")
            NotifGui.Name = "VoidHub_Notifs"
